...
import {
  useLoadScript,
  createStorefrontClient as createStorefrontClient$1,
  ...
  flattenConnection,
  RichText as RichText$1,
  ShopPayButton as ShopPayButton$1,
  useShopifyCookies,
...
function AnalyticsView(props) {
  ...
  useEffect(() => {
    console.log(
      '[h2:debug:AnalyticsView] - Publishing view event',
      type,
      viewPayload2,
    );
    if (!shop?.shopId) {
      console.error(
        '[h2:error:AnalyticsView] - shopId is required for analytics events. Make sure to pass the shop prop to <Analytics.Provider> or use <Analytics.Provider shop={shop}>.',
      );
      return;
    }
    viewPayload2 = {
...
function AnalyticsCollectionView(props) {
  return /* @__PURE__ */ jsx(AnalyticsView, {
    ...props,
...
function useCustomerPrivacy(props) {
  ...
  function emitCustomerPrivacyApiLoaded() {
    console.log(
      '[debug:emitCustomerPrivacyApiLoaded] - emitCustomerPrivacyApiLoaded hasEmitted',
      hasEmitted,
    );
  ...
  useEffect(() => {
    console.log(
      '[debug:useCustomerPrivacy] - useEffect: Observing customerPrivacy',
      window?.Shopify,
    );
            ...
            set(value2) {
              if (
                ...
                customShopify = {
                  ...
                  // override the customerPrivacy object in Shopify
  ...
  const setLoaded = {
    customerPrivacy: () => {
      if (withPrivacyBanner) {
        ...
        console.log(
          'useApisLoaded: Customer Privacy API loaded without Privacy Banner',
        );
        setApisLoaded(() => [true]);
      }
    },
...
function overrideCustomerPrivacySetTrackingConsent({customerPrivacy, config}) {
  console.log('overrideCustomerPrivacySetTrackingConsent', config);
...
function overridePrivacyBannerMethods({privacyBanner, config}) {
  console.log('overridePrivacyBannerMethods', config);
...
function getCustomerPrivacyRequired() {
  ...
  if (!customerPrivacy) {
    throw new Error(
      'getCustomerPrivacyRequired: Shopify Customer Privacy API not available',
    );
  }
...
function ShopifyAnalytics({consent, onReady, domain}) {
  ...
  useCustomerPrivacy({
    ...
    onVisitorConsentCollected: () => {
      ...
      console.log(
        'useCustomerPrivacy: Customer Privacy API loaded, onVisitorConsentCollected called',
      );
    },
    onReady: () => {
      console.log(
        'useCustomerPrivacy: Customer Privacy API loaded, onReady called',
      );
      setPrivacyReady(true);
    },
  });
  ...
  useEffect(() => {
    console.log(
      'ShopifyAnalytics: useEffect - shopifyReady',
      shopifyReady,
      'privacyReady',
      privacyReady,
    );
    if (shopifyReady && privacyReady) {
      ...
      onReady();
      console.log('ShopifyAnalytics: Shopify Analytics API loaded');
    }
  ...
  useEffect(() => {
    ...
    console.log(
      'ShopifyAnalytics: Subscribed to analytics events, initialized',
    );
...
function prepareBasePageViewPayload(payload) {
  const customerPrivacy =
    window.Shopify?.customerPrivacy || getCustomerPrivacyRequired();
...
function PerfKit({shop}) {
  ...
  useEffect(() => {
    ...
    subscribe2(AnalyticsEvent.PAGE_VIEWED, () => {
      console.log(
        '[h2:debug:PerfKit] - Page viewed event received, navigating PerfKit',
      );
...
var defaultAnalyticsContext = {
  ...
  publish: () => {
    console.log(
      'defaultAnalyticsContext: publish is no-op because canTrack is no-op by default',
    );
  },
  shop: null,
  subscribe: () => {},
  register: () => ({
    ready: () => {
      console.log('register: no-op default');
    },
  }),
  customerPrivacy: null,
  privacyBanner: null,
};
var AnalyticsContext = createContext(defaultAnalyticsContext);
...
function subscribe(event, callback) {
  ...
  console.log(
    `subscribe: adding callback for event ${event}`,
    callback.toString(),
  );
...
function publishEvent(event, payload) {
  (subscribers.get(event) ?? /* @__PURE__ */ new Map()).forEach(
    (callback, subscriber) => {
      try {
        ...
        if (typeof error === 'object' && error instanceof Error) {
          ...
          console.error(
            'publishEvent: Analytics publish error',
            error,
            subscriber,
          );
        }
...
function register(key) {
  ...
  return {
    ready: () => {
      registers[key] = true;
      console.log(
        `registered: ${key} is ready publishing events`,
        waitForReadyQueue.size,
        'events in queue',
      );
...
function AnalyticsProvider({
  ...
  return /* @__PURE__ */ jsxs(AnalyticsContext.Provider, {
    ...
    children: [
      ...
      !!shop &&
        !!currentCart &&
        /* @__PURE__ */ jsx(CartAnalytics, {cart: currentCart, setCarts}),
      !!shop &&
        consent.checkoutDomain &&
        /* @__PURE__ */ jsx(ShopifyAnalytics, {
          consent,
          onReady: () => {
            listenerSet.current = true;
            console.log(
              'ShopifyAnalytics:onReady shopifyCanTrack',
              typeof shopifyCanTrack,
              shopifyCanTrack(),
            );
            setCanTrack(
              customCanTrack ? () => customCanTrack : () => shopifyCanTrack,
...
async function runWithCache(
  ...
  if (cachedItem && typeof cachedItem[0] !== 'string') {
    const [{value: cachedResult, debugInfo: debugInfo2}, cacheInfo] =
      cachedItem;
...
async function fetchWithServerCache(
  url,
  requestInit,
  {
    cacheInstance,
...
function getFormInput(formData) {
  ...
  const {action, inputs} = cartFormInput
...
function cartCreateDefault(options) {
  return async (input, optionalParams) => {
    ...
    const {cartCreate, errors: errors2} = await options.storefront.mutate(
...
function cartLinesAddDefault(options) {
  return async (lines, optionalParams) => {
    const {cartLinesAdd, errors: errors2} = await options.storefront.mutate(
...
function cartDiscountCodesUpdateDefault(options) {
  return async (discountCodes, optionalParams) => {
    ...
    const {cartDiscountCodesUpdate, errors: errors2} =
...
var CART_DISCOUNT_CODE_UPDATE_MUTATION = (
  cartFragment = MINIMAL_CART_FRAGMENT,
) => `#graphql
...
function cartBuyerIdentityUpdateDefault(options) {
  return async (buyerIdentity, optionalParams) => {
    ...
    const buyer = options.customerAccount
      ? await options.customerAccount.getBuyer()
      : void 0;
    const {cartBuyerIdentityUpdate, errors: errors2} =
      await options.storefront.mutate(
...
var CART_BUYER_IDENTITY_UPDATE_MUTATION = (
  cartFragment = MINIMAL_CART_FRAGMENT,
) => `#graphql
...
function cartSelectedDeliveryOptionsUpdateDefault(options) {
  return async (selectedDeliveryOptions, optionalParams) => {
    const {cartSelectedDeliveryOptionsUpdate, errors: errors2} =
      await options.storefront.mutate(
...
var CART_SELECTED_DELIVERY_OPTIONS_UPDATE_MUTATION = (
  cartFragment = MINIMAL_CART_FRAGMENT,
) => `#graphql
...
function cartAttributesUpdateDefault(options) {
  return async (attributes, optionalParams) => {
    const {cartAttributesUpdate, errors: errors2} =
      await options.storefront.mutate(
...
var CART_ATTRIBUTES_UPDATE_MUTATION = (
  cartFragment = MINIMAL_CART_FRAGMENT,
) => `#graphql
...
function cartMetafieldsSetDefault(options) {
  return async (metafields, optionalParams) => {
    ...
    const {cartMetafieldsSet, errors: errors2} =
      await options.storefront.mutate(CART_METAFIELD_SET_MUTATION(), {
...
function cartMetafieldDeleteDefault(options) {
  return async (key, optionalParams) => {
    const ownerId = optionalParams?.cartId || options.getCartId();
    const {cartMetafieldDelete, errors: errors2} =
      await options.storefront.mutate(CART_METAFIELD_DELETE_MUTATION(), {
        variables: {
          input: {
            ownerId,
            key,
...
function cartGiftCardCodesUpdateDefault(options) {
  return async (giftCardCodes, optionalParams) => {
    ...
    const {cartGiftCardCodesUpdate, errors: errors2} =
      await options.storefront.mutate(
...
var CART_GIFT_CARD_CODE_UPDATE_MUTATION = (
  cartFragment = MINIMAL_CART_FRAGMENT,
) => `#graphql
...
var DEFAULT_GITHUB_CHANGELOG_URL =
...
var DEFAULT_CUSTOMER_API_VERSION = '2025-01';
...
function createCustomerAccountClient({
  ...
  async function query(query2, options) {
    ...
    return withSyncStack(
      fetchCustomerAPI({query: query2, type: 'query', ...options}),
      {
        logErrors,
      },
    );
...
var Script = forwardRef((props, ref) => {
  const {waitForHydration, src, ...rest} = props;
  const nonce = useNonce();
  if (waitForHydration)
    return /* @__PURE__ */ jsx(LazyScript, {src, options: rest});
...
function usePagination(connection, namespace = '') {
  ...
  const currentPageInfo = useMemo(() => {
    ...
    let pageStartCursor =
      !hydrogenHydrated || stateInfo?.startCursor === void 0
        ? connection.pageInfo.startCursor
        : stateInfo.startCursor;
    let pageEndCursor =
      !hydrogenHydrated || stateInfo?.endCursor === void 0
        ? connection.pageInfo.endCursor
        : stateInfo.endCursor;
    let previousPageExists =
      !hydrogenHydrated || stateInfo?.hasPreviousPage === void 0
        ? connection.pageInfo.hasPreviousPage
        : stateInfo.hasPreviousPage;
    let nextPageExists =
      !hydrogenHydrated || stateInfo?.hasNextPage === void 0
  ...
  const previousPageUrl = useMemo(() => {
    ...
    currentPageInfo.startCursor &&
      params2.set(cursorParam, currentPageInfo.startCursor);
  ...
  const nextPageUrl = useMemo(() => {
    ...
    currentPageInfo.endCursor &&
      params2.set(cursorParam, currentPageInfo.endCursor);
    return `?${params2.toString()}`;
        ...
        if (productWithVariants) {
          setResolvedVariants(
            productWithVariants instanceof Array
              ? productWithVariants
              : productWithVariants.product?.variants?.nodes || [],
          );
        }
      })
      .catch((error) => {
...
function VariantSelector({
  ...
  const selectedVariantOptions = selectedVariant
    ? selectedVariant?.selectedOptions?.reduce((selectedValues, item) => {
...
var RichText = function (props) {
  return /* @__PURE__ */ jsx(RichText$1, {
    ...props,
    components: {
      link: ({node}) =>
        /* @__PURE__ */ jsx(Link, {
